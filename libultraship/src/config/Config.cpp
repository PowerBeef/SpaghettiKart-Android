...
AudioBackend Config::GetCurrentAudioBackend() {
#ifdef __ANDROID__
    // ✅ Always use SDL on Android
    if (GetString("Window.AudioBackend", "") != "sdl") {
    ...
    if (backendName == "pulse" || backendName == "sdl") {
        return AudioBackend::SDL;
    }
...
#ifdef __ANDROID__
    // ✅ Always force OpenGL on Android
    const auto forcedBackend = WindowBackend::FAST3D_SDL_OPENGL;
    if (GetInt("Window.Backend.Id", -1) != static_cast<int>(forcedBackend)) {
        SetInt("Window.Backend.Id", static_cast<int>(forcedBackend));
